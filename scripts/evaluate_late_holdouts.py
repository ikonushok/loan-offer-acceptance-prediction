#!/usr/bin/env python
"""Evaluate CatBoost configs on overlapping late-period holdouts.

This is an L4 robustness evaluator:
- trains target models only on train_apps.csv;
- evaluates several future-like holdouts independently;
- does not train a final full-data model;
- does not predict test_apps.csv;
- does not build a submission.

Holdouts are intentionally allowed to overlap. Therefore no combined OOF
ROC-AUC is reported; use per-holdout AUC plus mean/std/min.
"""
...
def sanitize_for_json(value: Any) -> Any:
    """Recursively convert numpy scalars and NaN/inf values into JSON-safe values."""
...
def evaluate_late_holdouts(
    ...
    for i, holdout in enumerate(holdouts, start=1):
        ...
        valid_pool = Pool(train_x.iloc[valid_idx], label=y_valid, cat_features=cat_features)
    ...
    summary_rows = [
        {
            ...
            "train_rows": np.nan,
            "valid_rows": np.nan,
            "valid_positive_rate": np.nan,
            "best_iteration": np.nan,
            "roc_auc": float(auc_values.mean()),
        },
        {
            "holdout": "LATE_HOLDOUT_STD",
            "cutoff": "",
            "valid_end_requested": "",
            "train_start": "",
            "train_end": "",
            ...
            "valid_positive_rate": np.nan,
            "best_iteration": np.nan,
            "roc_auc": float(auc_values.std(ddof=0)),
        },
        {
            "holdout": "LATE_HOLDOUT_MIN",
            "cutoff": "",
            "valid_end_requested": "",
            "train_start": "",
            "train_end": "",
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        required=True,
        help="CatBoost JSON config compatible with scripts/baseline_catboost_time.py.",
    )
    parser.add_argument(
        "--holdouts-json",
        default="",
        help="Optional JSON list of holdouts with name/cutoff/optional valid_end.",
    )
    ...
    seed = int(cfg["seed"])
    model_cfg = cfg["model"]
    run_prefix = f"late_holdouts_{cfg['run_prefix']}"
    ...
    train_x, _test_x, y, categorical_cols, feature_cols, feature_manifest = prepare_features(
        train=train,
        test=test,
